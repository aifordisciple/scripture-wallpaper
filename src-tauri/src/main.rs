...
async fn init_database(app: AppHandle, db_path: String) -> Result<String, String> {
    ...
    if (count as usize) < SCRIPTURE_DATA.len() {
        // Insert missing verses (skip duplicates by content)
        for (content, reference) in SCRIPTURE_DATA.iter() {
            conn.execute(
                "INSERT OR IGNORE INTO scriptures (content, reference) VALUES (?1, ?2)",
                [*content, *reference],
            )
            .map_err(|e| e.to_string())?;
        }
...
#[tauri::command]
async fn get_random_scripture(app: AppHandle, db_path: String) -> Result<(String, String), String> {
    let path = resolve_path(&app, &db_path)?;
    let conn = Connection::open(&path).map_err(|e| e.to_string())?;
    ...
    let result = conn
        .query_row(
            "SELECT content, reference FROM scriptures LIMIT 1 OFFSET ?1",
            [offset],
...
async fn fetch_bing_daily(app: AppHandle, save_path: String) -> Result<String, String> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let img_resp = reqwest::blocking::get(&full_img_url)
...
async fn generate_wallpaper(
    ...
    let y_position = if image_height > 200 {
        (image_height - 150) as i32
    } else {
        50
    };
    let x_position = 100i32;
    ...
    draw_text_mut(&mut img, text_color, x_position, y_position, scale, &font, &text);
    ...
    let tsv_content = format!(
        ...
        date_str, text, inp.to_string_lossy()
    );
    tsv_file
...
/// Set system desktop wallpaper using Finder AppleScript (more reliable than System Events)
#[tauri::command]
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            font_path: "/System/Library/Fonts/STHeiti Medium.ttc".to_string(),
...
#[tauri::command]
async fn save_config(app: AppHandle, config_path: String, config: AppConfig) -> Result<String, String> {
    let path = resolve_path(&app, &config_path)?;
...
/// List all favorites
#[tauri::command]
async fn list_favorites(app: AppHandle, db_path: String) -> Result<Vec<(i64, String, String, String, String)>, String> {
    let path = resolve_path(&app, &db_path)?;
    let conn = Connection::open(&path).map_err(|e| e.to_string())?;

    let mut stmt = conn
        .prepare("SELECT id, content, reference, image_path, created_at FROM favorites ORDER BY id DESC")
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
async fn remove_favorite(app: AppHandle, db_path: String, favorite_id: i64) -> Result<String, String> {
    let path = resolve_path(&app, &db_path)?;
    let conn = Connection::open(&path).map_err(|e| e.to_string())?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let update_now = MenuItemBuilder::with_id("update_now", "立即刷新今日经文").build(app)?;
